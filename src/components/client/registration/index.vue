<template>
  <v-dialog v-model="registrationDialog" persistent max-width="800px">
    <v-card>
      <v-toolbar flat>
        <v-toolbar-title lass="headline">User Registration Form</v-toolbar-title>
...
export default {
  ...
  computed: {
    ...
    confirmPasswordErrors() {
      ...
      !this.$v.newUser.confirmPassword.required &&
        errors.push("Confirm Password is required.");

      !this.$v.newUser.confirmPassword.sameAsPassword &&
        errors.push("Confirm password should be same as password above.");
      return errors;
    },
    emailErrors() {
      const errors = [];
  ...
  methods: {
    submit() {
      ...
      this.$store
        .dispatch("submitClientRegistration", this.newUser)
        .then(res => {
